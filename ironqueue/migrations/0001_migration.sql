CREATE TABLE ironqueue.jobs (
    ...
    -- holds every Rust writer to. Unbounded, `ironqueue.job_is_stuck` computed
    ...
    -- The clock stuck-job recovery reads. `ironqueue.job_is_stuck` tests
...
CREATE UNIQUE INDEX jobs_dedupe_key_idx ON ironqueue.jobs (queue, dedupe_key)
...
-- queue.
CREATE INDEX jobs_dequeue_idx ON ironqueue.jobs (queue, priority, scheduled_at, id)
    WHERE status = 'queued';
CREATE INDEX jobs_expires_idx ON ironqueue.jobs (queue, expires_at, id)
    WHERE expires_at IS NOT NULL;
CREATE INDEX jobs_page_idx ON ironqueue.jobs (queue, enqueued_at DESC, id DESC);
CREATE INDEX jobs_dashboard_status_page_idx ON ironqueue.jobs
    (queue, kind, status, enqueued_at DESC, id DESC);
CREATE INDEX jobs_dashboard_name_page_idx ON ironqueue.jobs
...
CREATE INDEX jobs_dashboard_name_prefix_idx ON ironqueue.jobs
    (queue, kind, status, lower(name) text_pattern_ops, enqueued_at DESC, id DESC);
CREATE INDEX jobs_dashboard_ready_idx ON ironqueue.jobs (queue, scheduled_at, id)
    WHERE status = 'queued';
CREATE INDEX jobs_dashboard_terminal_idx ON ironqueue.jobs
...
CREATE INDEX jobs_active_idx ON ironqueue.jobs (queue, status)
...
-- The indexed text columns carry `ironqueue.jobs`'s bounds, for the reason that
...
--
-- Exactly `ironqueue.jobs`'s bounds, including where they are *loose*:
...
CREATE TABLE ironqueue.cron_occurrences (
    queue        text NOT NULL CHECK (octet_length(queue) BETWEEN 1 AND 255),
...
CREATE INDEX cron_occurrences_expiry_idx
    ON ironqueue.cron_occurrences (queue, expires_at);

-- `queue`, `dedupe_key` and `name` are bounded for the reason given above
-- `ironqueue.cron_occurrences`; `(queue, dedupe_key)` is this table's primary key
-- too. `expression` is not indexed, so it needs no bound of its own.
CREATE TABLE ironqueue.cron_schedules (
    queue          text NOT NULL CHECK (octet_length(queue) BETWEEN 1 AND 255),
    dedupe_key     text NOT NULL CHECK (octet_length(dedupe_key) <= 255),
    name           text NOT NULL CHECK (octet_length(name) BETWEEN 1 AND 255),
...
CREATE TABLE ironqueue.workers (
    id           uuid PRIMARY KEY,
...
CREATE INDEX workers_queue_idx ON ironqueue.workers (queue, expires_at, id);
CREATE INDEX workers_dashboard_page_idx ON ironqueue.workers (queue, started_at, id);
...
--   2. Its owner is provably gone — the `ironqueue.workers` lease that covered
...
-- `ironqueue.jobs` tuple as a composite datum per row and re-ran the lease lookup
...
-- `LEFT JOIN`. `ironqueue.workers.id` is the primary key, so that join yields at
...
CREATE FUNCTION ironqueue.job_is_stuck(
    j                ironqueue.jobs,
    grace_ms         bigint,
...
CREATE FUNCTION ironqueue.job_page_keys(
    p_queue     text,
    p_kind      text,
    p_statuses  text[],
...
CREATE FUNCTION ironqueue.job_page_keys_by_name(
    p_queue     text,
    p_kind      text,
    p_statuses  text[],
    p_name      text,
