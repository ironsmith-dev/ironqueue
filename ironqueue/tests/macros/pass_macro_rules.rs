...
use ironqueue::{CronDefinition, JobType};
...
macro_rules! define_typed_job {
    ($payload:ty) => {
        #[ironqueue::job(name = "generated_typed", max_attempts = 2)]
...
macro_rules! define_returning_job {
    ($name:ident, $($ret:tt)+) => {
        #[ironqueue::job(name = "generated_returning", max_attempts = 2)]
        async fn $name(_: ()) -> $($ret)+ {
            Ok(1)
        }
    };
...
macro_rules! define_aliased_job {
    ($ret:ident) => {
        #[ironqueue::job(name = "generated_aliased", max_attempts = 2)]
...
macro_rules! define_bounded_job {
    ($name:ident, $label:literal, $($ty:ty: $bound:path),*) => {
        #[ironqueue::job(name = $label, max_attempts = 2)]
...
macro_rules! define_returning_cron {
    ($($ret:tt)+) => {
        #[ironqueue::cron("*/5 * * * *", name = "generated_returning_cron")]
