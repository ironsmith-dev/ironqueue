//! The `#[ironqueue::job]` / `#[ironqueue::cron]` expansions, kept as pure token
...
/// registration marker.
enum Mode {
    /// `#[ironqueue::job]`: first param is the payload, the rest are extractors.
    Job,
    /// `#[ironqueue::cron("...")]`: every param is an extractor; the payload is
...
impl Mode {
    fn attr_name(&self) -> &'static str {
        match self {
            Mode::Job => "#[ironqueue::job]",
            Mode::Cron { .. } => "#[ironqueue::cron]",
        }
    }
...
/// Expands `#[ironqueue::cron("expr", ...)]`, parsing the cron expression at
...
fn expand(mode: Mode, attrs: JobAttrs, item: TokenStream) -> syn::Result<TokenStream> {
    ...
    // bound `u32: IntoJobResult` is not satisfied` underlined `#[ironqueue::job]`
    ...
    let decode_args = match &mode {
        Mode::Job => {
            ...
            // against `#[ironqueue::job]` — one mistake, three errors, pointing at
    ...
    // `#[ironqueue::job]` and named nothing the user could act on. It is not
    ...
    // underlines that body rather than `#[ironqueue::job]`. At `call_site()` the
...
impl ItemAttrs {
    ...
    /// reaches this expansion at all — `#[ironqueue::job(bogus_key = 1)]
...
/// Whether a `#[cfg_attr(...)]` meta can resolve to a `#[cfg(...)]` — directly
/// in its tail, or through a nested `cfg_attr`. Such an attribute can remove
/// the item it lands on, so it has to be copied onto every generated item
/// exactly as a bare `cfg` is; one that cannot yield a `cfg` leaves the
/// function in place under every configuration and stays where it was written.
/// Anything unparseable answers `false` and is left for rustc to diagnose on
/// the function.
fn cfg_attr_wraps_cfg(meta: &Meta) -> bool {
    ...
    // The first element is the predicate; the rest are the attributes it
    // resolves to.
    parts.iter().skip(1).any(|inner| inner.path().is_ident("cfg") || cfg_attr_wraps_cfg(inner))
}
...
/// for `#[ironqueue::cron]`, at worker startup. The raw-identifier prefix is
...
fn runtime_crate_path() -> TokenStream {
    match crate_name("ironqueue") {
        Ok(found) => found_crate_path(found),
        Err(_) => quote!(::ironqueue),
    }
}
...
fn found_crate_path(found: FoundCrate) -> TokenStream {
    match found {
        ...
        FoundCrate::Name(name) if matches!(name.as_str(), "crate" | "self" | "super" | "Self" | "_") => {
        ...
        // `ironqueue` exposes this self-alias so the same absolute path works in
        // library code, doctests, and package integration tests.
        FoundCrate::Itself => quote!(::ironqueue),
    }
}
...
fn validate(mode: &Mode, func: &ItemFn) -> syn::Result<()> {
    ...
    for input in &func.sig.inputs {
        ...
        // arguments were supplied` pointing at `#[ironqueue::job]`, naming
        ...
        // A borrowed parameter declares no generic parameter when its lifetime
        // is elided, so it slips past the generics check above and fails instead
        // as "missing lifetime in associated type", helpfully suggesting a
        // lifetime on an `impl` block the author cannot see. The payload is
        // deserialized from the job row and extractors are built per attempt, so
        // neither can borrow. Only a top-level reference is refused here: a
        // lifetime *inside* the type can be legitimate, as `Cow<'static, str>`
        // is a valid payload today.
        if let Type::Reference(reference) = &*argument.ty {
            return Err(syn::Error::new_spanned(
                reference,
                format!(
                    "{} functions cannot take a borrowed parameter; the payload is \
                     deserialized from storage and extractors are built per attempt, \
                     so both must be owned",
                    mode.attr_name()
                ),
            ));
        }
    ...
    // The same elided-lifetime hole as argument position, and the output has a
    // stricter requirement still: it is stored as JSON and read back, so it must
    // be `DeserializeOwned`.
    if let ReturnType::Type(_, ty) = &func.sig.output
    ...
    if matches!(mode, Mode::Job) && func.sig.inputs.is_empty() {
        return Err(syn::Error::new_spanned(
            &func.sig.ident,
            "#[ironqueue::job] functions need a payload as their first parameter; \
...
/// `ironqueue`'s own `MAX_DURATION_MS`, so the limit has a single source of truth
/// even though this crate cannot depend on `ironqueue`.
///
...
/// `#[ironqueue::job(...)]` attribute — while every attribute error raised here
...
fn duration_bound_assertions(attrs: &JobAttrs, runtime: &TokenStream) -> Vec<TokenStream> {
    attrs
        ...
        .iter()
        .map(|(field, ms, span)| {
            let message = format!("{field} exceeds ironqueue's maximum supported duration");
            quote_spanned! {*span=>
                const _: () = ::core::assert!(
...
/// this very macro generates for `#[ironqueue::job] async fn config(...)` — turns it
...
mod tests {
    ...
    /// "ironqueue", ... }` is a dependency key Cargo accepts and `proc_macro_crate`
    ...
    /// Expanding *inside* `ironqueue` — its doctests, its own library code — has
    /// no dependency entry to read a name from, and resolves through the
    /// `extern crate self as ironqueue` alias instead.
    #[test]
    fn test_runtime_crate_path_uses_the_self_alias_inside_ironqueue() {
        let path = found_crate_path(FoundCrate::Itself);
        assert_eq!(compact(&path.to_string()), "::ironqueue");
    }
    ...
    fn test_expands_minimal_job() {
        ...
        assert!(flat.contains("impl::ironqueue::JobTypeforsend_email"), "{out}");
        assert!(flat.contains("impl::ironqueue::JobDefinitionforsend_email"), "{out}");
        assert!(flat.contains("typeArgs=SendEmail;"), "{out}");
        assert!(
            flat.contains("typeOutput=<anyhow::Result<()>as::ironqueue::__private::IntoJobResult>::Output;"),
            "{out}"
        );
    ...
    fn test_expands_extractors_positionally() {
        ...
        assert!(flat.contains("<JobState<Pool>as::ironqueue::FromJobContext>::from_context(&__ctx)"), "{out}");
        assert!(flat.contains("<JobContextas::ironqueue::FromJobContext>::from_context(&__ctx)"), "{out}");
    ...
    fn test_expands_all_config_attrs() {
        ...
        assert!(
            flat.contains("::ironqueue::JobRetention::For(::core::time::Duration::from_millis(3600000u64))"),
            "{out}"
        );
        assert!(flat.contains("__config.retry_delay=::core::time::Duration::from_millis(500u64);"), "{out}");
        assert!(flat.contains("::ironqueue::JobRetryBackoff::Exponential{max:::core::option::Option::Some"), "{out}");
        assert!(flat.contains("__config.priority=-1i16;"), "{out}");
        // Unit return type maps through IntoJobResult for ().
        assert!(flat.contains("typeOutput=<()as::ironqueue::__private::IntoJobResult>::Output;"), "{out}");
    }
    ...
    fn test_moves_deprecation_to_the_generated_job_type() {
        ...
        assert!(flat.contains("#[allow(deprecated)]impl::ironqueue::JobTypeforlegacy"), "{out}");
        assert!(flat.contains("#[allow(deprecated)]impl::ironqueue::JobDefinitionforlegacy"), "{out}");
    ...
    fn test_copies_lint_attributes_onto_every_generated_item() {
        ...
        for item in [
            "implundocumented{",
            "impl::ironqueue::JobTypeforundocumented",
            "impl::ironqueue::JobDefinitionforundocumented",
        ] {
            assert!(
        ...
        assert!(
            flat.contains("#[allow(missing_docs)]#[deny(clippy::pedantic)]#[allow(non_camel_case_types)]"),
            "{out}"
        );
        assert!(
            flat.contains(
    ...
    fn test_lint_attributes_reach_the_impls_that_name_the_user_s_types() {
        for out in [
            ...
            assert!(flat.contains("#[allow(deprecated)]impl::ironqueue::JobTypefor"), "{out}");
    ...
    fn test_cfg_gated_parameters_are_refused() {
        for gate in [quote!(#[cfg(any())]), quote!(#[cfg_attr(test, cfg(any()))])] {
            let err = expand_job(
                quote!(),
                quote! {
                    async fn j(_: (), #gate _metrics: u32) {}
                },
            )
            .unwrap_err();
            assert!(err.to_string().contains("cannot gate a parameter with `#[cfg]`"), "{err}");
        }
    }
    ...
    /// labelled every job's telemetry `__ironqueue_inner`, losing the handler
    ...
    fn test_hidden_handler_keeps_the_user_s_identifier() {
        ...
        let flat = compact(&out);
        assert!(
            !flat.contains("__ironqueue_inner"),
        ...
        let flat = compact(&out);
        assert!(!flat.contains("__ironqueue_inner"), "{out}");
    ...
    /// `#[ironqueue::job] async fn config(...)` triggers for every *other* job in
    ...
    fn test_inner_attributes_stay_on_the_handler_body() {
        ...
        let flat = compact(&out);
        let struct_at = flat.find("pubstructwork;").unwrap_or_else(|| panic!("{out}"));
        for inner in [
            "#![allow(unused_variables)]",
            ...
            assert!(inner_at > struct_at, "{inner} must stay inside the hidden handler: {out}");
        }
        // The outer doc still moves to the struct, as before.
        let doc_at = flat.find("Outerdocumentationdescribesthejob.").unwrap_or_else(|| panic!("{out}"));
    ...
    fn test_lowers_a_forbid_to_a_deny_on_the_generated_struct() {
        ...
        let flat = compact(&out);
        assert!(flat.contains("#[deny(non_camel_case_types)]#[allow(non_camel_case_types)]"), "{out}");
    ...
    fn test_lowers_an_expect_to_an_allow_on_the_generated_struct() {
        ...
        assert!(flat.contains("#[allow(missing_docs)]#[allow(non_camel_case_types)]"), "{out}");
    ...
    fn test_rejects_impl_trait_in_return_position() {
        ...
        assert_eq!(
            err.to_string(),
            "#[ironqueue::job] functions cannot use `impl Trait` in return position; \
             use a concrete type"
        );
        ...
        assert_eq!(
            err.to_string(),
            "#[ironqueue::cron] functions cannot use `impl Trait` in return position; \
             use a concrete type"
        );
    }
    ...
    fn test_job_config_default_is_absolutely_qualified() {
        ...
        let flat = compact(&out);
        assert!(flat.contains("<::ironqueue::JobConfigas::core::default::Default>::default()"), "{out}");
    ...
    fn test_job_rejects_the_cron_only_revision_key() {
        ...
        .unwrap_err();
        assert_eq!(err.to_string(), "`revision` is only valid on #[ironqueue::cron]");
    }
    ...
    fn test_expands_cron_with_extractors_only() {
        ...
        assert!(flat.contains("pubfnjob()->::ironqueue::JobBuilder<cleanup>"), "{out}");
        assert!(flat.contains("::ironqueue::JobBuilder::new(())"), "{out}");
        // The schedule is baked in.
        assert!(flat.contains("impl::ironqueue::CronDefinitionforcleanup"), "{out}");
    ...
    fn test_cron_rejects_bad_input() {
        ...
        .unwrap_err();
        assert!(err.to_string().contains("#[ironqueue::cron] functions must be async"), "{err}");
        ...
        .unwrap_err();
        assert!(err.to_string().contains("#[ironqueue::cron] functions cannot be unsafe"), "{err}");
    }
}
